...
from aveli.browser import Browser, StalePage
    ...
    try:
        ...
        browser.evaluate(
          ...
          <select id="category" aria-label="Category">
            <option>All</option><option value="same" disabled>Unavailable</option><option value="same">Design</option>
          </select><a id="download" href="data:text/plain,report" download>Download report</a>
        ...
        assert [a["label"] for a in actions if a["kind"] == "select"] == ["Category → Design"]
        download = next(a for a in actions if a["label"] == "Download report")
        assert download["effect"] == "download"
        passed.append("native controls expose only supported operations and safe values")
        passed.append("download links expose a distinct policy effect")
        ...
        passed.append("native dropdown selects the exact observed option identity")
