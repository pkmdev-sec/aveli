...
function render() {
  ...
  $("screenshot").src = page.screenshot ? `data:image/jpeg;base64,${page.screenshot}` : "";
  ...
  $("targets").innerHTML = [...targets.values()].map((a,i) => {
    ...
    return `<div class="target ${index === selectedIndex ? 'selected' : ''}" data-aveli-action="${index}" style="left:${100*a.rect.x/page.w}%;top:${100*a.rect.y/page.h}%;width:${100*a.rect.w/page.w}%;height:${100*a.rect.h/page.h}%"><span>${index}</span></div>`;
  }).join('');
...
$("choices").addEventListener("pointerover", (event) => {
  const id = event.target.closest("[data-aveli-action]")?.dataset.aveliAction;
  document
    .querySelectorAll(".target")
    .forEach((t) =>
      t.classList.toggle(
        "selected",
        t.dataset.aveliAction === id || t.dataset.aveliAction === state?.decision?.target?.split(':')[0],
      ),
    );
});
$("choices").addEventListener("pointerleave", () =>
  document
    ...
    .forEach((t) =>
      t.classList.toggle(
        "selected",
        t.dataset.aveliAction === state?.decision?.target?.split(':')[0],
      ),
    ),
);
